...
import (
	...
	"os/signal"
	"strings"
	"github.com/damageless/go-webrtc"
	"strconv"
...
func receiveChat(msg string) {
	...
	for k, v := range itemsMap {
		...
		audioData[key] = v.(float64)
		// fmt.Println(FloatToString(v.(float64)))
		i++;
	}

	ringBuffer.Value = audioData
}
...
func process(nframes uint32) int {
	...
	if (ringBuffer.Value == nil) {
		return 0
	}

	for i := range samples {
		samples[i] = jack.AudioSample(ringBuffer.Value.([2048]float64)[i])
	}
...
func setupJack() {
	ringBuffer = ring.New(256)

	fmt.Println("Setting up jack.")
	...
	fmt.Println("Finished setting up jack.")
